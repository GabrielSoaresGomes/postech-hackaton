...
@Slf4j
public class ListMedicalCareWithPriorityUseCase implements UseCase<ListMedicalCareRequestDTO, List<MedicalCare>> {
    private final MedicalCareGateway medicalCareGateway;

    public ListMedicalCareWithPriorityUseCase(MedicalCareGateway medicalCareGateway) {
    ...
    public List<MedicalCare> execute(ListMedicalCareRequestDTO request) {
        ...
        return this.medicalCareGateway.findAll(PageRequest.of(request.page(), request.size(), sortByPriority), Boolean.TRUE);
    }
}
